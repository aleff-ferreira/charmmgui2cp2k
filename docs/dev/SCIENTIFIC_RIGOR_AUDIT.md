...
**Method:** A systematic review across 12 subsystems. Each candidate finding was independently re-checked from three angles — is it already guarded elsewhere in the code, is the underlying chemistry actually wrong, and can it be reproduced in real generated output — and kept only if it survived all three.
**Findings:** 37 confirmed after discarding 6 false alarms — 5 Critical (C1–C5), 13 High (H1–H13), 16 Medium (M1–M16), 3 Low (L1–L3).
...
The pipeline is well-engineered on the *housekeeping* axes that were easiest to falsify: per-atom charge bookkeeping, torsion/1-4 preservation, NONBONDED_PARM_INDEX arithmetic, and E16.8 quantization were all probed and **dismissed as false alarms** (no topology finding survived). That is a genuinely good signal — the obvious failure modes are guarded.
...
Severities below reflect the re-check; where the initial and re-checked severity differed, the adjustment is noted inline.
...
*Fix:* Add `'&FORCE_EVAL/&DFT/&XC/&VDW_POTENTIAL/&PAIR_POTENTIAL/TYPE (DFTD4)': (8,1)` to `CP2K_KEYWORD_MIN_VERSION`; validate it in the `dispersion_scheme_override` branch (~20466), raising `ValueError` if the detected version is below 8.1.
...
*Fix:* Call `classify_forbidden_link_bond()` per link in `BoundaryPhase.validate()`; on any forbidden pair append a `correction`-severity record and return `(False, [remedy message: widen QM region / supply bonded FF])`.
...
*Fix:* Add `MethodPhase.validate()` mirroring CLI lines 19398-19421; block advancement (or record a blocking `correction`) when uncovered elements exist and no CP2K data dir is available.
...
*Consequence:* Out-of-bounds QM indices propagate three ways: phantom QM/MM links in `detect_link_bonds`, invalid `MM_INDEX` written to `&QM_KIND` (13718-13723) → CP2K runtime failure, and `'X'` element fallback (2998, 3085) that corrupts electron counting and the spin decision.
*Fix:* Pass `topo`/`natom` into `extract_qm_from_pdb`; filter to `[1, natom]` and warn+drop out-of-range indices (pattern of 2985-2988), or validate at the 17431 call site before building `qm_set`.
...
*Fix:* Read `bond_arr[k+2]`, map to multiplicity via AMBER encoding, forbid non-single cuts (or route to an explicit `pi_bond_cuts` confirmation list).
...
*Fix:* In `generate_qm_kinds`, `error()` + `sys.exit(1)` when `unresolved` is non-empty; or promote to a fatal spin-gate flag.

**H3 — Peptide C–N amide bonds treated as single bonds (≈10% α error)**
`charmmgui2cp2k.py:252` (`('C','N'): 1.35  # 1.47/1.09`).
*Consequence:* Peptide C–N is ~1.33 Å (π-character); using 1.47 Å places the H-cap at ~1.472 Å instead of ~1.330 Å, mis-coupling QM/MM electrostatics and distorting effective backbone stiffness in protein-dynamics studies. Error ≈10.7%.
*Fix:* Detect backbone peptide bonds (C(=O)–N) and use α_peptide ≈ 1.22; document that backbone cuts should be avoided.

**H4 — Aromatic C–C link bonds use single-bond IMOMM α (≈7–8% error)**
`charmmgui2cp2k.py:249-286` (entry `('C','C'): 1.38`; no aromatic entry).
*Consequence:* For conjugated/aromatic systems the H-cap is misplaced by ~7.8% (correct α_aromatic ≈ 1.28), perturbing the embedding field, dipoles, and electronic-state ordering — directly relevant to aromatic active sites.
*Fix:* Add aromatic C–C α ≈ 1.28 with ring/aromaticity detection, or document that rings should be fully enclosed in QM.
...
*Fix:* Add a total-charge balance check; WARN below ~1e-6 e, `RuntimeError` above ~1e-5 e.
...
*Consequence:* Transition-metal QM regions (AMBIGUOUS_REQUIRES_USER) and parity-violating user multiplicities never reach the formal `STRICT_GATE_EXIT_CODE=3` path; CI/batch sees inconsistent exit codes and the formal concerns ledger is blind to a real rigor failure.
*Fix:* Pass the `spin_decision` dict into `collect_generation_scientific_concerns`; add a concern when `risk_flags` non-empty or `parity_consistent` is False.
...
*Fix:* Pass `parity_consistent` to the gate; append `('spin_parity', ...)` concern when False.
...
*Fix:* Add `test_dftd4_requires_cp2k_8_1_in_gating_dict`, `test_dftd4_cli_override_validates_version`, and a version-gate coherence test.
...
*Fix:* Raise `ValueError` listing out-of-range indices `[1..ncoord]`.
...
*Fix:* Raise an error when `rcut_relaxed` (or require an explicit `--force-reduced-rcut` override).
...
*Fix:* Validate against `QM_BASIS_SET_PRESETS` (9547-9554) or a GTH naming pattern; WARN for custom names so the user knows a `BASIS_SET_FILE_NAME` is required.
...
*Fix:* Add `WorkflowPhase.validate()` enforcing `em_max_iter≥1`, `mm_nvt_steps≥1`, `mm_npt_steps≥1`, `md_timestep>0`, `md_temperature>0`, valid ensemble.
...
*Fix:* Warn/confirm when `md_timestep > 0.5` under QM/MM.

### MEDIUM

**M1 — No hard floor blocking physically nonsensical RCUT–cell combinations**
`charmmgui2cp2k.py:10519` — a 0.5 Å cell drives `effective_rcut` to 0.15 Å.
*Fix:* `if effective_rcut < 1.0: raise ValueError(...)`.

**M2 — `extract_qm_from_pdb` element extraction may produce invalid element symbols**
`charmmgui2cp2k.py:3042-3047` — regex yields e.g. `'CA'` (Cα → calcium), `'HB'` (non-element).
*Consequence:* Silent mis-element → wrong GTH pseudopotential (Ca q2 vs C q4), wrong core configuration.
*Fix:* Validate against `SYMBOL_TO_ATOMIC_NUM`; fall back to topology atomic numbers or require the PDB element column (76-78).
...
*Fix:* Error+exit in non-interactive mode; prompt to confirm in interactive.
...
*Fix:* Topology connectivity check that QM atoms form connected components respecting bonds; warn/confirm/error per mode.

**M5 — Inconsistent covalent radii for forbidden transition metals**
`charmmgui2cp2k.py:3305-3310` (radii for MN/FE/CO/NI/CU/ZN) vs `325-342` (same elements forbidden).
...
*Fix:* Remove forbidden metals from `COVALENT_RADII_ANG`, or guard `expected_covalent_bond_length()` to return `None` for forbidden elements.
...
*Fix:* Add cross-channel identity `|Σ residual.removed + Σ link.M1_CHARGE_E − Σ qm_atom.original_charge| ≤ tol`; pass `qm_indices`/`topo` in.

**M7 — Parity check fails to warn when electron count unknown (interactive mode)**
`charmmgui2cp2k.py:19613-19626`; `validate_multiplicity_parity` returns `(None, msg)` at 5231.
...
*Fix:* Change to `if parity_ok is not True:` and message that count is unknown.

**M8 — Identical parity-check logic bug in non-interactive mode**
`charmmgui2cp2k.py:19691-19699`.
...
*Fix:* `if parity_ok is not True:`; distinguish "parity-inconsistent" from "parity-unverifiable" in the message.

**M9 — Duplicate M1 frontier atoms not gated as a concern**
`charmmgui2cp2k.py:3398-3424` (detector), `17744-17773` (warn only).
...
*Fix:* Collect duplicate-M1 as a gate concern.

**M10 — Unresolved elements in electron count not gated as a concern**
`charmmgui2cp2k.py:4492-4650`, `5110-5128` (added to `risk_flags`, never to the gate).
...
*Fix:* Pass `qm_e_meta` to the gate; concern when `unresolved_elements` non-empty.

**M11 — Inconsistent exit-code convention across rigor checks**
`charmmgui2cp2k.py:19666-19674` & `19699` (`sys.exit(1)`) vs `21070-21077` (`STRICT_GATE_EXIT_CODE=3`).
...
*Fix:* Route all rigor failures through the formal gate; standardize on exit 3 under `--strict`.

**M12 — Hardcoded `EWALD_PRECISION` independent of QM cell and RCUT**
`charmmgui2cp2k.py:13714`.
...
*Fix:* Scale precision with effective RCUT or expose as a policy parameter.

**M13 — Warnings not blocking generation, easily overlooked**
`charmmgui2cp2k.py:20997, 21004` (~15-20 non-blocking `warnings.append`).
...
*Fix:* Promote RCUT/box-limited warnings to errors, or require an explicit `--accept-reduced-rcut` flag.
...
*Fix:* Check candidate coverage during recommendation; recommend the broadest-coverage basis or disabling ADMM.

**M15 — MGRID CUTOFF/REL_CUTOFF advisory-only, no hard floor**
`charmmgui2cp2k.py:11755-11827` (WARN at 11779-11788, never aborts); defaults safe (500/60 at 12924).
...
*Fix:* Optional `--strict-convergence` hard error below the recommended floors, or a CRITICAL warning requiring `--accept-under-converged-mgrid`.

**M16 — Timestep mismatch between MM stages and QM/MM warmup not validated**
`charmmgui2cp2k.py:18261-18301`.
...
*Fix:* Warn (non-blocking) when `mm_timestep > warmup_timestep`.

### LOW

**L1 — Default 100 Å MM box may mask genuine system-size problems**
`charmmgui2cp2k.py:12178, 17344` (also second warning at 20994).
*Consequence:* A box-less RST7 silently gets a 100 Å cube that may not match the real system; warning missable in batch.
*Fix:* Smaller/safer default with a stronger warning, or require `--box-override` / treat as error in strict mode.
...
*Fix:* Rewrite the comment to state explicitly that double/aromatic/triple bonds pass silently.

**L3 — EXT_RESTART velocity restoration for stage 40 is implicit, not explicit**
`charmmgui2cp2k.py:14073-14080, 14092-14099` (`prod_restart_vel=True` outside `md_state_flags`).
...
*Fix:* Fold `restart_vel` into `md_state_flags`.

---

## 3. Considered but dismissed
...
- **`generated_output` dimension reported 0 findings** — this likely reflects *thin coverage* of the final emitted `.inp` as an integrated whole, not a clean bill of health. A golden-file / CP2K-`--check`-parse test of a full generated input is a genuine gap.
