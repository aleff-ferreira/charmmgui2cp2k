...
def cp2k_input_provenance_header(system_model, dft_config, run_config):
    """Deterministic provenance comment header embedded at the top of every input.

    Records the key generation parameters *inside* the CP2K input itself so the
    file is self-describing and reproducible (FAIR R1.2; "configuration log" of
    the Ten Simple Rules for usable software).  Only deterministic, input-derived
    values are included — environment/dependency versions and timestamps stay in
    run_provenance.txt so the assembled input remains byte-stable for regression
    and so identical inputs always produce identical output.
    """
    s, d, r = system_model, dft_config, run_config
    qm_atoms = sum(len(v) for v in (s.qm_elements or {}).values())
    return [
        f"! ============================================================\n",
        f"! Generated by charmmgui2cp2k v{__version__}\n",
        f"! Automated CHARMM-GUI/AMBER -> CP2K QM/MM input generator.\n",
        f"! Parameters below are recorded for reproducibility (FAIR R1.2).\n",
        f"! Full decision log, environment versions and timestamp:\n",
        f"!   run_provenance.txt (same output directory).\n",
        f"!\n",
        f"!   run_type              : {r.run_type}\n",
        f"!   functional            : {d.functional}\n",
        f"!   basis_set             : {d.basis_set}\n",
        f"!   admm                  : {'on (' + str(d.admm_aux_basis) + ')' if d.use_admm else 'off'}\n",
        f"!   dispersion            : {d.dispersion_scheme}\n",
        f"!   boundary_charge_scheme: {d.boundary_charge_scheme}\n",
        f"!   qm_atoms              : {qm_atoms}\n",
        f"!   link_bonds            : {len(s.link_bonds or ())}\n",
        f"!   qm_charge             : {s.qm_charge}\n",
        f"!   multiplicity          : {s.multiplicity}\n",
        f"!   qm_cell_abc [Ang]     : {s.qm_cell_abc}\n",
        f"! ============================================================\n",
        "\n",
    ]
...
def _assemble_qmmm_input(system_model, dft_config, run_config):
    ...
    out = []
    out.extend(cp2k_input_provenance_header(system_model, dft_config, run_config))
